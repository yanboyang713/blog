...
1.  ****Distance between clusters****: This is the shortest distance between two clusters. For two clusters \\(C\_i\\) and \\(C\_j\\), it can be computed as: \\(d(C\_i, C\_j)\\)

2.  ****Distance within clusters****: This is the maximal intra-cluster distance. For a cluster \\( C\_i \\), it can be computed as:

\\(\max\_{x, y \in C\_i} d(x, y)\\)
